...
namespace _5.TAREA_1
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            double sumaCalif = 0, sumaNotasSalon=0, totalAlumnos = 0, promedio, califMin = 10, califMax = 0;
            ...
            Console.WriteLine("********************");
            double[] caliMinSalon = new double[salones];
            double[] caliMaxSalon = new double[salones];
            ...
            for (i = 0; i < salones; i++)
            {
                ...
                for (j = 0; j < calificaciones[i].Length; j++)
                {
                    ...
                    //Acumulamos las califiaciones de toda la escuela:
                    sumaCalif += calificaciones[i][j];

                    //Cada salon
                    sumaNotasSalon += calificaciones[i][j];

                    if (calificaciones[i][j]<califMin)
                    {
                        califMin = calificaciones[i][j];
                    }
                    caliMinSalon[i] = califMin;

                    if (calificaciones[i][j] > califMax)
                    {
                        califMax = calificaciones[i][j];
                    }
                    caliMaxSalon[i] = califMax;
                }

                promedioSalon[i] = sumaNotasSalon / calificaciones[i].Length;
            }
            ...
            for(i = 0;i < salones; i++)
            {
                ...
                Console.WriteLine("La califiación mínima es: {0} y maxima {1}", caliMinSalon, caliMaxSalon);
                Console.WriteLine("Promedio: {0} ", promedioSalon);
            }

            //Mostramos los resultados para la escuela
